...
def G14(s):
    ...
    model_path = os.path.join('/content/drive/MyDrive/Colab_Notebooks/OpenShape/G14_model','model.pt')
...
def L14(s):
    ...
    model_path = os.path.join('/content/drive/MyDrive/Colab_Notebooks/OpenShape/L14_model','model.pt')
    s = torch.load(model_path)
...
def B32(s):
    ...
    model_path = os.path.join('/content/drive/MyDrive/Colab_Notebooks/OpenShape/B32_model','model.pt')
    s = torch.load(model_path)
    dic = model.load_state_dict(module(s, 'pc_encoder'))
    print(dic)
    return model
...
def load_pc_encoder(name):
    model_path = os.path.join("/content/drive/MyDrive/Colab_Notebooks/OpenShape/G14_model", name, 'model.pt')
...
def eval_ood_md2sonn(opt, config, s):
    ...
    s = torch.load(model_path)
    model = G14(s)
    state_dict = module(s['state_dict'], 'module')
